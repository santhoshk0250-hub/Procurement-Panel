...
interface LocalAddon {
  ...
  name: string;
  description: string;
  price: string; // keep as string (matches requested payload)
  category: AddonCategory;
  isAvailable: boolean;
  selected: boolean;
}
...
export default function AddFoodServiceFormMobile() {
  ...
  const [data, setData] = useState<FoodFormData>(BLANK);
  ...
  const [descEditor, setDescEditor] = useState<EditorState>(() =>
    htmlToEditorState(BLANK.description)
  );
  ...
  // Create Add-on modal state (local only)
  ...
  // Selected summary (existing + local)
  ...
  // Auto placement: open upward if near bottom
  useEffect(() => {
  ...
  // Fetch existing add-ons (read-only)
  useEffect(() => {
    let cancelled = false;
  ...
  }, [data, newBanner, existingBannerUrl]);
  ...
  const addIngredient = (v: string) =>
    setData((p) => ({ ...p, ingredients: [...p.ingredients, v] }));
  ...
  // Toggle existing add-on selection
  ...
  const saveLocalAddon = () => {
    ...
    if (!category) {
      setCreateErr("Category is required");
      return;
    }
    if (priceStr === "" || Number.isNaN(Number(priceStr)) || Number(priceStr) < 0) {
      setCreateErr("Price must be a valid, non-negative number");
      return;
    }
    ...
    <form className="min-h-screen bg-gray-50" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-base font-semibold text-gray-900 truncate">
                Add Food Item — {data.name || "New"}
              </h1>
              <p className="text-[11px] text-gray-500 truncate">Fill required sections and submit</p>
            </div>
            <button
              type="button"
              onClick={resetAll}
      ...
      <main className="max-w-3xl mx-auto p-4 sm:p-6 pb-36">
              ...
              <div ref={addonsDropdownRef} className="relative">
                ...
                {addonsOpen && !addonsLoading && !addonsError && (
                  ...
                  >
                    ...
                    {/* List (LOCAL first, then EXISTING) */}
        ...
        {step.key === "media" && (
          <SectionCard
            title="Media"
            subtitle="Upload a banner (required) and gallery images."
            icon={<ImageIcon className="size-5 text-blue-600" />}
            requiredHint
          >
            {/* Banner */}
            <h3 className="text-sm font-semibold text-blue-900 mb-2">Banner (required)</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {existingImageUrls.map((url) => (
                <div key={url} className="relative aspect-square rounded-xl overflow-hidden border border-gray-300 bg-gray-50 shadow-sm">
              ...
              {newImages.map((img, idx) => (
                <div key={img.preview} className="relative aspect-square rounded-xl overflow-hidden border-2 border-dashed border-blue-400 bg-blue-50">
