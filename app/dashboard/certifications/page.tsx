...
import { Edit, Save, X, Plus, Trash2, Loader2, FileText } from "lucide-react";
import { getInstance, deleteInstance } from "@/lib/swr";
import TinyMCETextEditor from "@/components/TinyMCETextEditor";

type Certification = {
  _id?: string;
  title: string;
  description: string; // ✅ HTML string from TinyMCE
  imageUrl: string;
  documentUrl: string;
};
...
export default function CertificationsPage() {
  ...
  const hasNewImage = (index: number) => Boolean(imageFiles.get(index));
  const hasNewDoc = (index: number) => Boolean(docFiles.get(index));

  const getImagePreviewSrc = (index: number, cert: Certification) => {
  ...
  const handleRemove = async (index: number) => {
    ...
    // indices shift — easiest safe cleanup
    imagePreviews.forEach((url) => URL.revokeObjectURL(url));
  ...
  const handleSave = async () => {
    try {
      ...
      for (let i = 0; i < certifications.length; i++) {
        ...
        formData.append("description", cert.description); // ✅ TinyMCE HTML

        // image
        const imgFile = imageFiles.get(i);
        if (imgFile) formData.append("image", imgFile, imgFile.name);
        else formData.append("imageUrl", cert.imageUrl);

        // document (any type)
        const docFile = docFiles.get(i);
        if (docFile) formData.append("document", docFile, docFile.name);
        else formData.append("documentUrl", cert.documentUrl);
        ...
        if (cert._id) {
          ...
          await fetch(`${process.env.NEXT_PUBLIC_API_BASE}certifications`, {
            method: "POST",
            body: formData,
          });
        }
  ...
  if (isEditing) {
    return (
      <div className="fixed inset-0 z-50 bg-white flex flex-col md:relative md:z-auto md:bg-gray-50 md:p-2 md:sm:p-4 md:md:p-6 md:-mx-3 md:sm:mx-0">
        ...
        <div className="flex-1 overflow-y-auto md:max-w-6xl md:mx-auto md:w-full">
          <section className="bg-white md:rounded-lg md:shadow-sm px-4 py-4 md:p-3 md:sm:p-6 min-h-full md:min-h-0">
            ...
            <div className="sticky top-0 bg-white border-b border-gray-200 -mx-4 px-4 py-3 mb-4 flex gap-2 z-10 md:static md:bg-transparent md:border-0 md:-mx-0 md:px-0 md:py-0 md:mb-3 md:sm:mb-4 md:flex-row md:justify-between md:items-center">
              <h2 className="text-base font-semibold md:text-lg md:sm:text-2xl">
                Certification List
              </h2>
            ...
            {activeSection && (
              <div className="space-y-4 sm:space-y-6">
                {certifications.map((cert, index) => (
                  <div
                    key={index}
                    className="border p-4 sm:p-5 rounded-lg bg-gray-50"
                  >
                    <div className="space-y-3 sm:space-y-3">
                      ...
                      <div>
                        ...
                        {(cert.imageUrl || imageFiles.get(index)) && (
                          <div className="mb-3">
                            ...
                            <img
                              src={getImagePreviewSrc(index, cert)}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const oldPreview = imagePreviews.get(index);
                      ...
                      <div>
                        ...
                        {cert.documentUrl && !hasNewDoc(index) && (
                          <a
                            href={cert.documentUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mb-3 inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:py-2 text-blue-600 hover:text-blue-700 active:text-blue-800 hover:bg-blue-50 active:bg-blue-100 rounded-lg border border-blue-200 text-sm touch-manipulation transition-colors"
                          >
        ...
        {/* TinyMCE styling (optional – similar to your other page) */}
        <style jsx global>{`
          .tox {
          ...
          {activeSection && (
            <div className="space-y-3 sm:space-y-6">
              {certifications.map((cert, index) => (
                <div
                  key={index}
                  className="border p-3 sm:p-5 rounded-lg bg-gray-50"
                >
