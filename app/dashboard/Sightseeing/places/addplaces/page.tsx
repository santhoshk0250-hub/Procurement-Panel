...
export default function AddPlaceMobile() {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      fd.append(
        "payload",
        JSON.stringify({ place: cooked, createdAt: new Date().toISOString() })
      );
            ...
            <div className="rounded-xl border border-gray-200 p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Type *" required>
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (val === "__add__") { setAddingType(true); return; }
                    ...
                    <div className="flex items-stretch gap-2">
                      <input
                        ...
                        onKeyDown={(e) => {
                          ...
                          else if (e.key === "Escape") { setCustomType(""); setAddingType(false); }
                        }}
                      />
                      <button
                        type="button"
                        ...
                        className={`size-10 grid place-items-center rounded-full text-white transition ${
                          submitting || !customType.trim() ? "bg-green-300 cursor-not-allowed" : "bg-green-600 hover:bg-green-700 active:bg-green-800"
                        }`}
                        title="Save type"
                      ...
                      <button
                        type="button"
                        onClick={() => { setCustomType(""); setAddingType(false); }}
                        disabled={submitting}
                        className={`size-10 grid place-items-center rounded-full text-white transition ${
                  ...
                  <p className="text-[11px] text-gray-500 mt-2">Can’t find it? Choose <span className="font-medium">“Add a custom type…”</span></p>
                </Field>
  ...
  function saveCustomType() {
    ...
    if (!val) return;
    // No cast; TYPE_OPTIONS ensures the select displays it
    setPlace({ type: val });
    setCustomType("");
    setAddingType(false);
  }
