...
const UPDATE_BASE = joinUrl(API_BASE, "/activity/update"); // PUT /activity/:id
const AFTER_SAVE_REDIRECT = "/dashboard/Activities";

/** Array key helper for repeated fields */
const USE_BRACKETS_FOR_ARRAYS = true;
const ARRAY = (k: string) => (USE_BRACKETS_FOR_ARRAYS ? `${k}[]` : k);

/** ---------- Local types just for the editor form ---------- */
type TabName =
  | "Activity Details"
  | "Price Configuration"
  | "Schedule"
...
type DateSurchargeDraft = {
  mode: "single" | "range";
  startDate: string; // YYYY-MM-DD for UI
  endDate: string; // YYYY-MM-DD for UI
  surchargeAmount: string; // keep as string in UI
...
type DraftActivity = {
  ...
  dateSurcharges: DateSurchargeDraft[];
...
/** ---------- Date helpers ---------- */
// Accepts "2025-11-11T00:00:00.000Z" or "2025-11-11" and returns "2025-11-11"
...
// Accepts "2025-11-11" and returns "2025-11-11T00:00:00.000Z"
const dateInputToISO = (v?: string | null) => {
  if (!v) return "";
  if (!/^\d{4}-\d{2}-\d{2}$/.test(v)) return "";
  return new Date(`${v}T00:00:00.000Z`).toISOString();
};

/** ---------- FormData builder (for PUT) ---------- */
function buildEditFormData(params: {
  ...
  const { payload, imageFiles, videoFiles } = params;
  ...
  // keep existing media URLs that user didn't remove
  (payload.keepImageUrls || []).forEach((u) => fd.append(ARRAY("keepImageUrls"), u));
  (payload.keepVideoUrls || []).forEach((u) => fd.append(ARRAY("keepVideoUrls"), u));
...
async function putActivityBinary({
  baseUrl,
  id,
  body,
  signal,
...
export default function EditActivityForm() {
  ...
  // Guard: if store is empty, bounce back to listing
  useEffect(() => {
    if (!activity) {
  ...
  // Provide a minimal UI fallback to avoid rendering crashes
  if (!activity) {
    return null;
  }
  ...
  /** ---------- Seed locations (optional demo list) ---------- */
  ...
  /** ---------- Seed the form from store.activity ---------- */
  const toDraft = (a: typeof activity): DraftActivity => ({
    name: a.name ?? "",
    description: a.description ?? "",
    destination: a.destination ?? "",
    coverImage: a.coverImage ?? null,

    vendorPrice: String(a.vendorPrice ?? ""),
    sellingPrice: String(a.sellingPrice ?? ""),
    taxRate: String(a.taxRate ?? ""),
    taxIncluded: !!a.taxIncluded,

    dateSurcharges: (a.dateSurcharges || []).map((s) => ({
      mode: s.mode,
      startDate: isoToDateInput(s.startDate), // normalize to YYYY-MM-DD for inputs
      endDate: isoToDateInput(s.endDate), // normalize to YYYY-MM-DD for inputs
    ...
    durationType: a.durationType || "hrs",
  ...
  const [form, setForm] = useState<DraftActivity>(toDraft(activity));
  ...
  /** ---------- Surcharge helpers ---------- */
  const blankSurcharge: DateSurchargeDraft = {
    mode: "single",
    startDate: "",
    endDate: "",
    surchargeAmount: "",
  ...
  useEffect(() => {
    if (activeTab === "Surcharges" && (form.dateSurcharges?.length ?? 0) === 0) {
  ...
  const updateSurchargeAt = <K extends keyof DateSurchargeDraft>(
    index: number,
    key: K,
    value: DateSurchargeDraft[K]
  ) => {
    setForm((prev) => {
      const next = [...prev.dateSurcharges];
      const curr = { ...next[index], [key]: value } as DateSurchargeDraft;
  ...
  const validateAll = (draft: DraftActivity) => {
    const errors: string[] = [];
    ...
    for (let i = 0; i < surges.length; i++) {
      ...
      if (s.startDate && end && dateToNum(s.startDate) > dateToNum(end)) {
    ...
    const hasAnyImage = (draft.images?.length ?? 0) > 0;
    if (!hasAnyImage) {
  ...
  const handleNextOrSave = async () => {
    ...
    const { ok, errors, focus } = validateAll(form);
    if (!ok) {
      alert(errors.join("\n"));
    ...
    // Separate keep-URLs (existing http/https) from newly added blob previews
    const keepImageUrls = (form.images || []).filter((u) => !u.startsWith("blob:"));
    const keepVideoUrls = (form.videos || []).filter((u) => !u.startsWith("blob:"));
    ...
    const body = buildEditFormData({
      payload: {
        ...
        dateSurcharges: (form.dateSurcharges || []).map((s) => {
          const startISO = dateInputToISO(s.startDate);
          const endISO = s.mode === "single" ? startISO : dateInputToISO(s.endDate);
          return {
            mode: s.mode,
            startDate: startISO,
            endDate: endISO,
    ...
    try {
      const id =
        (typeof activity._id === "string" ? activity._id : (activity._id as any)?.$oid) ?? "";

      if (!id) throw new Error("Missing activity id for editing");

      await putActivityBinary({
        baseUrl: UPDATE_BASE,
        id,
        body,
      });
      ...
      // Optional: clear previews (blob revocation best-effort)
      [...form.images, ...form.videos].forEach((u) => {
  ...
  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    {form.videos.map((src, idx) => (
      ...
      >
        <video src={src} controls className="w-full h-48 object-contain bg-black" />
        <button
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white shadow-md rounded-2xl p-4 sm:p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-xl sm:text-2xl font-bold">Edit Activity</h1>
            <div className="flex gap-2">
              <button
                onClick={() => router.back()}
